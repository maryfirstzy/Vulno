...
# Google Colab Root pathing
input_file = "/content/BTC.txt"
output_file = "/content/found.txt"

if not os.path.exists(input_file):
    print(f"[-] ERROR: '{input_file}' not found in Colab root directory.")
    print("[!] Please upload your file using the folder icon on the left panel.")
else:
    print("[*] Reading signatures from Colab path...")
    sigs = []
    skipped_count = 0
    ...
    print(f"[+] Loaded {len(sigs)} signatures successfully. (Skipped lines: {skipped_count})")
    ...
    keys_recovered = 0
    print("[*] Sweeping data matrix for matching R values...")
                        ...
                        if sig1['msg_hash'] != sig2['msg_hash']:
                            ...
                            if raw_pk:
                                ...
                                log_entry = (
                                    ...
                                    f"Source lines        : Line {sig1['id']} & Line {sig2['id']}\n"
    ...
    if keys_recovered > 0:
        print(f"\n[SUCCESS] Completed! {keys_recovered} keys written to '{output_file}'.")
        print("[*] Click the file refresh button in your left folder panel to find and download 'found.txt'.")
    else:
        print(f"\n[~] Swept matrix. Processed {len(sigs)} entries, but exactly 0 exploitable nonces were active.")
